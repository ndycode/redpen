...
import { getConfig, getConfigFile, saveConfig, detectStack } from '../lib/config.js';
import { getVersion, fuzzyMatch, copyToClipboard, colors as c } from '../lib/utils.js';
import { getProgress, getBranch, saveProgress, markDone, markSkipped, undoLast } from '../lib/progress.js';
import { getRunOrder, resolvePrompt, getPromptName, getPromptCategory, getPromptContent } from '../lib/prompts.js';
import { renderSidebar, handleSidebarKey, SidebarState, CategoryData } from './sidebar.js';
...
export class InitTUI {
    ...
    render() {
        ...
        if (this.step === INIT_STEPS.CONFIRM) {
            ...
            out.push('');
            out.push(pad(`  ${fg(C.muted)}Config: ${fg(C.text)}${getConfigFile()}${term.reset}`, W));
            out.push('');
        } else {
            const options = this.getCurrentOptions();
...
export class TUI {
    ...
    updateFilter() {
        if (!this.searchQuery) {
            ...
            this.filteredItems = this.items.filter((item) => fuzzyMatch(this.searchQuery, getPromptName(item)));
        }
    ...
    run() {
        ...
        markDone(item);
        this.advanceToNextIncomplete(getProgress(), 'Copied ✓');
    }
    ...
    skip() {
        ...
        markSkipped(item);
        this.advanceToNextIncomplete(getProgress(), 'Skipped');
    }
